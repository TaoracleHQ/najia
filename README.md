...
维基文库不是校勘本，它自己也有讹误。以 ctext.org 的《周易》（版本记为**武英殿十三經注疏**）交叉比对了 61 卦（ctext 限流未取到 3 卦），19 处单字分歧中绝大多数是公认异体（藜/蔾、佑/祐、亨/享、他/它，两个方向都有），**仅 2 处判为维基文库的错，已在 `tools/build-guaci.py` 的 `CORRECTIONS` 表中修正**：

| 卦 | 原 | 改为 | 依据 |
| --- | --- | --- | --- |
| 地雷复 初九 | 不**复远** | 不**远复** | 字序颠倒；紧随其后的象曰作「不远之复」，可自证 |
| 地雷复 初九 | 无**袛**悔 | 无**祗**悔 | 袛为内衣，此处无义 |

修正写在生成脚本里而非直接改数据，重新生成时会自动复用，并在目标字符串消失时报错。
